...
public class View {
  ...
  private Matrix4f getLookAtMatrix() {
    Matrix4f lookAt = new Matrix4f();
    lookAt.lookAt(new Vector3f(new Vector3f(0, 120, 300)), new Vector3f(0, 0, 0), new Vector3f(0, 1, 0));
    return  lookAt;
  }
...
//    modelView.peek().translate(0, 0, -20);//.mul(trackballTransform);
    modelView.peek().mul(getLookAtMatrix())
